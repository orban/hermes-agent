...
class ShellFileOperations(LintMixin, SearchMixin, FileOperations):
    ...
    def search(self, pattern: str, path: str = ".", target: str = "content",
        ...
        # A wrapper failure outranks the path verdict: the probe never ran, so the
        # working directory is at fault rather than ``path``.
        if probe.cwd_error:
            return SearchResult(error=probe.cwd_error)
        # Checked before the stat-failure branch below: "unresponsive" carries neither
        # "exists" nor "not_found", so that branch would otherwise swallow it.
        if "unresponsive" in exists_probe:

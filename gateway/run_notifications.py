...
_DURABLE_CLAIM_OPS = {
    ...
    "defer": ("defer_event_delivery", "Could not defer unadmitted completion claim"),
    "complete": ("complete_event_delivery", "Could not acknowledge durable completion claim"),
}
...
class GatewayNotificationsMixin:
    ...
    async def _preflight_completion_delivery(self, evt: dict) -> "_CompletionClaim":
        ...
        if evt_type in _DURABLE_EVENT_TYPES and not evt.get("task_failure_notice"):
    ...
    async def _deliver_completion_notification(
        ...
        try:
            ...
            if claim.claim_id:
                self._settle_durable_claim(operation, evt, claim.claim_id)
            for sibling, claim_id in sibling_claims:
                if claim_id:
                    self._settle_durable_claim(operation, sibling, claim_id)
